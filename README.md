...
本节用"海天味业 DCF 估值"作为完整案例，对照 Wind Skill 同题任务的产出，呈现本套件的工作流、产出形态和方法论选择。本案例**仅用于说明工具差异和模型质量，不构成对海天味业股票的投资建议**；估值结果会随取数日、市场价格、WACC、Beta、Capex、EBITDA Margin 和终值假设变化。

### 案例背景

- **标的**：海天味业（603288.SH）
- **基准日**：2026 年 6 月 2 日
- **当日股价**：约 35.31—36.01 元（盘中波动）
- **数据源**：两套工具均使用 Wind MCP / Wind MCP Skill 取数
- **触发命令**：`/financial-analysis:dcf 海天味业`（本套件）vs 自然语言触发 Wind DCF Skill

### 一、产出形态对比（本套件最核心的差异点）

| 产出物 | 本套件（`/financial-analysis:dcf`）| Wind Skill |
|---|---|---|
| 估值结论报告（Markdown）| ✅ 估值摘要 v2 | ✅ DCF 估值报告 |
| 项目卡 + 假设说明 + 风险清单 | ✅ 独立文件，分目录归档 | ❌ |
| Wind 原始数据归档 | ✅ `01_原始资料/Wind原始数据汇总.md` | ❌ |
| 可比公司分析表（Excel，公式式）| ✅ `海天味业_Comps_v1.xlsx` | ❌ |
| DCF 模型（Excel，多 Sheet 公式式）| ✅ `海天味业_DCF_v1.xlsx`（假设/Bear/Base/Bull/汇总/敏感性 6 张表）| ❌ |
| 敏感性矩阵（Excel 内）| ✅ WACC × g 7×6 矩阵 | ✅ 4×4 矩阵（在 Markdown 内） |
| 可交互、可调假设的模型 | ✅ 改一个黄色单元格，6 张 sheet 联动重算 | ❌ 结论写死，无法调参 |
| 标准项目文件夹结构 | ✅ `01_原始资料/02_研究过程/03_项目计划/04_成果文件` | ❌ |

**形态本质**：

- **Wind Skill 输出"结论"**：一份 Markdown 报告，所有数字都是 AI 算好后写死的静态值。读者觉得 Beta 不对、WACC 不对、增速假设偏保守，只能重跑一遍，无法自行调参。
- **本套件输出"可工作的估值系统"**：Excel 文件中所有 DCF 计算（折现、终值、权益桥、每股价值、敏感性矩阵）都是单元格公式，不是 Python 算好后写入的静态数。读者认为 Beta 该取 0.9 而非 0.75，在假设表改一个黄色单元格，6 张 sheet 全部联动重算，立即看到新的每股价值。

**对国资投资、产业投资场景的实际意义**：

| 场景 | Wind Skill | 本套件 |
|---|---|---|
| 领导问"如果 WACC 取 7% 呢？" | 无法回答，需重新跑 AI | 在 Excel 改一个数，秒出结果 |
| 投决会上有人质疑 Beta | 只能口头解释 | 现场改 Beta，投影展示变化 |
| 半年后跟踪估值更新 | 无模型可复用，从头来 | 换一组 Wind 新数据，模型框架直接复用 |
| 交接给同事继续跟踪 | 只有一份静态报告 | 文件夹结构完整，假设/过程/成果分离 |

### 二、估值结果对比

| 维度 | Wind Skill | 本套件 |
|---|---:|---:|
| 中性每股价值 | 22.2 元 | 36.75 元 |
| 当前股价 | 35.31 元 | 36.01 元 |
| 安全边际判断 | 小幅高估 | 基本到位（+2%）|
| 估值区间 | 18—38 元 | 22.8—58.7 元 |
| 企业价值 EV | 1,089 亿 | 1,906 亿 |
| 终值占 EV | 77% | 85.8% |
| 投资建议口径 | 不建议新增，回调至 30 元以下可考虑 | 市场定价基本充分，缺乏安全边际 |

两套结果方向一致（均指向"无明显安全边际"），但中性数值差异显著。差异的根因在方法论选择，不在工具能力本身——下文逐项呈现。

### 三、方法论差异与根因

| 参数 | Wind Skill | 本套件 | 差异说明 |
|---|---:|---:|---|
| **Beta** | **1.11**（Wind 3 年实测） | **0.75**（行业代理）| Wind 实测受 2023—2024 食安舆情扰动；行业代理无实证，纯主观 |
| **Ke（权益成本）** | 8.36% | 6.30% | 差 2.06 个百分点 |
| **WACC** | **8.30%** | **6.10%** | 差 2.20 个百分点（最大单一差异源）|
| **FCFF 口径** | OCF − Capex（简化）| NOPAT + D&A − Capex − ΔWC（完整）| 完整口径可与预测假设解耦 |
| **基期 FCFF** | 52.68 亿（2024 实际）| 由 EBITDA Margin 推导 | 基期选择不同 |
| **预测期** | 2025—2029（5 年）| 2026—2030（5 年）| 起始年份不同 |
| **终值法** | 仅 Gordon | Gordon + EV/EBITDA 退出倍数两法平均 | 本套件更稳健 |
| **可比公司** | 提及未展开 | 完整 4 家 EV 推导 + 倍数表 | 本套件更完整 |
| **交叉验证** | 无 | 有（DCF 隐含倍数 vs 同业中位数）| 本套件独有 |
| **少数股东权益** | 扣除 5.09 亿 | 未单独扣除 | Wind 更规范 |

**核心差异定位**：**WACC 8.30% vs 6.10%** 贡献了约 60% 的估值差异。原因是 Beta 不同——Wind 实测 1.11 把海天定性为高于市场波动的标的，与消费龙头属性不符（很可能受 2023—2024 食品安全舆情期股价异常拖累）；本套件 0.75 是行业代理（A 股调味品板块 0.65—0.85 的中位），更符合长期画像但缺乏实证。

**两套都有改进空间**：

- Wind Skill：应用 5 年 Beta 替代 3 年，平滑舆情冲击；
- 本套件：Wind 接口返回 null 时，应主动调用更长窗口的 Beta（如 3 年、5 年）或同业实测平均，而不是直接采用纯代理值。

### 四、估值结果哪个更合理？

**中性的判断是：真值大概率在 28—34 元之间**，对应 WACC 7.0%—7.5%、Beta 0.85—0.95 的折中假设区间。两份报告各自的中性值（22.2 元、36.75 元）都偏向假设的一端。

但这不影响两个共同结论：

1. **当前股价处于估值合理区间**——无论哪套假设，都不存在明显的安全边际或显著高估；
2. **DCF 对长期假设高度敏感**——终值占 EV 比例均超过 75%（健康区间 50—70%），显著高于警戒线，**单一 DCF 结论不应作为投资决策的唯一依据**；
3. **必须配合可比公司倍数交叉验证**——同业 EV/EBITDA 中位数 19.27× 对应每股约 36 元，与本套件 Base 接近，与 Wind Skill 中性 22.2 元偏离较大。

### 五、本套件的工作流验证清单

本案例同时验证了本套件 7 项工作流要求：

1. ✅ 开场说明"使用 Claude 金融套件中文版的 `/financial-analysis:dcf` 命令组织 DCF 估值流程；Wind MCP 仅作为数据源"；
2. ✅ A 股财务、行情、Beta、可比公司数据全程优先使用 Wind MCP Skill，未先进行 Web Search；
3. ✅ Wind 仅作数据源，未触发 Wind 的 `dcf-model` Skill；
4. ✅ Excel 中 DCF 折现、终值、权益桥、每股价值和敏感性表以单元格公式表达，Python 仅负责写入工作簿；
5. ✅ 主动剔除 ST 加加（连续亏损，倍数无意义），可比池 5 家调整为 4 家并说明理由；
6. ✅ Beta 取数失败（Wind 返回 null）时主动声明缺口，采用行业代理并在报告中显式标注；
7. ✅ 自检发现 Base 情景终值占 EV 85.8% 远超健康区间，在摘要中以"重要提示"形式警示读者，未隐瞒模型局限。
...
海天味业DCF估值/
├── 01_原始资料/
│   └── Wind原始数据汇总.md          # 历史财务、市场数据、可比池数据
├── 02_研究过程/
│   ├── 项目卡_v1.0.md               # 基础档案、任务设定、采集清单
│   ├── 关键假设与风险.md            # 三情景假设逻辑、风险与缓释
│   └── build_dcf_model.py           # Python 写入脚本（不含计算逻辑）
├── 03_项目计划及管理/
└── 04_成果文件/
    ├── 海天味业_Comps_v1.xlsx        # 可比公司分析（公式式）
    ├── 海天味业_DCF_v1.xlsx          # DCF 模型（6 张 sheet，公式式）
    └── 海天味业_估值摘要_v2.md       # 最终摘要报告
```

### 七、给读者的建议

本案例的核心价值不在于"哪个工具算得对"，而在于：

1. **不要把任何 AI 估值结果当作终极结论**——所有 DCF 都对假设极度敏感，工具只是降低重复劳动；
2. **优先选择"可调参的模型"而非"固定结论"**——尤其是国资投资、投决会、领导汇报场景，可调参的 Excel 模型才能支撑现场答辩和事后跟踪；
3. **Beta 缺失或异常时，必须显式声明**——任何用代理值的地方都应在报告中标注，不要让读者把代理值当作实测值；
4. **DCF 必须配合可比公司倍数**——单一 DCF 结论的置信度有限，可比倍数提供市场定价锚点；
5. **终值占比超过 70% 必须警示**——这是模型可靠性下降的明确信号，读者需要知道这件事；
6. **不构成投资建议**——本案例展示工具差异和模型质量，不对海天味业股票走势作任何判断或推荐。

> **数据来源**：万得 Wind 金融数据服务（行情、财务、股本、Beta、可比公司）。
> **维护说明**：如发现案例数据过时或方法论有更优实践，欢迎通过 GitHub Issue 反馈。
