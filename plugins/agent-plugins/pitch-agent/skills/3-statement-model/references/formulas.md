...
**重要提示：** 除非用户另有指定，否则请使用本参考文档中列出的公式。
...
**重要提示：** 毛利必须从 Net Revenue 计算，而非 Gross Revenue。
...
| 术语 | 定义 |
|------|------|
| Gross Revenue | 扣除前的总收入 |
| Net Revenue | Gross Revenue - 退货 - 折让 - 折扣 |
| Cost of Revenue | 可归属于已售商品/服务的直接成本 |
| Gross Profit | Net Revenue - Cost of Revenue |

**注意：** 始终使用 Net Revenue（在大多数财务报表上也称为 "Net Sales" 或简称为 "Revenue"）作为盈利能力计算的起点。Gross Revenue 会高估真实的顶线表现。
